...
metadata_file = '/home/adrian/projects/brca2/metadata/brca2_metadata_expression_mode.tsv'
...
###
### 3. read metadata
### test 3 or 5 minus minus samples, but 6 is too poor quality
###
metadata = read.table(metadata_file, header=TRUE, sep="\t")
metadata = metadata[c(1:31), ]
metadata = metadata[-c(8, 16),]
View(metadata)
tag = 'quality_strict'
...
### 4. run contrasts
###
local_samples = metadata$sampleID
...
dds$genotype <- relevel(dds$genotype, ref="plus plus")
